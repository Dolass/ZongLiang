...
Class Sdcms_Create
	...
	Public Function Sitemap_Google(t0,t1,t2)
		...
		Set Rs=Conn.Execute("Select "&t3&" id,ClassUrl,htmlname,lastupdate From View_Info Where ispass=1 order by lastupdate desc,id desc")
		While Not Rs.Eof
		t5=t5&"<url>"&vbcrlf
		Select Case Sdcms_Mode
			...
			Case "1"
				t5=t5&"<loc>"&sdcms_weburl&Sdcms_Root&"html/"&Rs(1)&Rs(0)&Sdcms_FileTxt&"</loc>"&vbcrlf
			Case "2"
	...
	Public Sub Create_Info_Show(t0)
		...
		Dim Show_I_Style,Show_I_ClassName,Show_I_Show_Temp,diy_ptitle
		FileName="index"&Sdcms_FileTxt
		Set Temp=New Templates
		...
		IF Rs_N.Eof Then
			Echo "<center>提示：信息未通过审核或不存在，不能查阅<br /><a href='"&Sdcms_WebUrl&"'>返回首页</a></center>"
		...
		Temp.Label "{sdcms:class_allclassid}",0
		Temp.Label "{sdcms:class_classurl}",""
		Temp.Label "{sdcms:class_url}",""
		Temp.Label "{sdcms:class_followid}",""
		Temp.Label "{sdcms:class_position}",""
		...
		For I=ubound(partentids) To 0 Step -1
			...
			IF Not Rs_P.Eof Then
				...
				End Select
				If diy_ptitle="" Then diy_ptitle = Rs_P(1)
		...
		Temp.Label "{sdcms:nav_title}",diy_ptitle
		IF Sdcms_Mode=2 Then
