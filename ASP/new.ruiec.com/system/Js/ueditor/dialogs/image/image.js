                            ...
                            if ( !list.children.length ) {
                                ajax.request(editor.options.imageManagerPath, {
                                    ...
                                    onsuccess:function ( xhr ) {
                                        ...
                                        g( "imageList" ).innerHTML = !length ? "&nbsp;&nbsp;当前未上传过任何图片或配置路径错误！" : "";
                                        for ( var i = 0, ci; ci = imageUrls[i++]; ) {
                                            var img = document.createElement( "img" );
                                            var div = document.createElement( "div" );
                                            div.appendChild( img );
...
//                                            };
											...
											//img.setAttribute( i < 35 ? "src" : "lazy_src", editor.options.MY_ROOT_PATH + ci );
											img.setAttribute( i < 35 ? "src" : "lazy_src", ci );
                                            //img.setAttribute( "data_ue_src", editor.options.UEDITOR_HOME_URL + ci );
											//img.setAttribute( "data_ue_src", editor.options.MY_ROOT_PATH + ci );
											img.setAttribute( "data_ue_src", ci );

                                        }
                                    },
                                    onerror:function () {
                                        g( "imageList" ).innerHTML = "糟糕，图片读取失败了！"+editor.options.imageManagerPath;
