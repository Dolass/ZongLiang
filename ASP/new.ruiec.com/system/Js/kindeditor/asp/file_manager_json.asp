...
'rootPath = "../attached/"
rootPath = "/upload/"
If Check_Folder(rootPath) Then
	'Create_Folder(uploadPath)
Else
	Create_Folder(rootPath)
End If
...
If Not isEmpty(dirName) Then
	If instr(lcase("image,images,flash,media,file"), dirName) < 1 Then
		Response.Write "Invalid Directory name.a"
		Response.End
	End If
...
For Each dir in folder.SubFolders
	isDir = True
	hasFile = (dir.Files.count+dir.SubFolders.count > 0)
	filesize = 0
	isPhoto = False
	filetype = ""
	filename = dir.name
