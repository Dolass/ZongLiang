...
function PlaceFlag()
  if Result="Search" Then
	If Keyword<>"" Then
		Response.Write "视频：列表 -> 检索 -> 关键字：<font color='red'>"&Keyword&"</font>"
	Else
		Response.Write "视频：列表 -> 检索 -> 关键字为空(显示全部视频)"
	End If
  else
    if SortPath<>"" then
      Response.Write "视频：列表 -> <a href='ImageList.asp'>全部</a>"
	  TextPath(SortID)
	else
      Response.Write "视频：列表 -> 全部"
	end if
  end if
end function
%>
...
<form name="formSearch" method="post" action="Search.asp?Result=Image">
  <tr>
    <th height="22" sytle="line-height:150%">【视频检索及分类查看】</th>
  </tr>
  <tr>
    <td class="forumRow">关键字：<input name="Keyword" type="text" value="<%=Keyword%>" size="20"> <input name="submitSearch" type="submit" value="搜索视频"></td>
  </tr>
  <tr>
...
<form action="DelContent.asp?Result=Image" method="post" name="formDel">
  <tr>
      <th width="5%">ID</th>
      <th width="70%" align="left">视频标题</th>
      <th width="5%">排序</th>
      <th width="10%">发布时间</th>
      <th width="5%">操作</th>
      <th width="5%">选择</th>
...
function GetVideoList()
	...
	If request.QueryString("Result")="Search" and request.QueryString("Keyword")<>"" Then				'搜索关键字
		SqlWhere=" VideoNameCh LIKE '%"&request.QueryString("Keyword")&"%'"	'where
	End If
	...
	Else
		Do While Not rs.eof
			...
			if StrLen((rs("VideoNameCh")))>40 Then
				Response.Write "<td nowrap title='"&rs("VideoNameCh")&"' class=""forumRow""><a href='VideoEdit.asp?Result=Modify&ID="&rs("ID")&"''>"&StrLeft(rs("VideoNameCh"),37)&"</a></td>"
			Else
				Response.Write "<td nowrap title='"&rs("VideoNameCh")&"' class=""forumRow""><a href='VideoEdit.asp?Result=Modify&ID="&rs("ID")&"''>"&rs("VideoNameCh")&"</a></td>"
			end if 
			Response.Write "<td nowrap class=""forumRow"">"&rs("Sequence")&"</td>"
			Response.Write "<td nowrap class=""forumRow"">"&rs("AddTime")&"</td>"
			Response.Write "<td align=""center""nowrap class=""forumRow""><a href='VideoEdit.asp?Result=Modify&ID="&rs("ID")&"'>修改</a></td>"
...
function ImageList()
  ...
  dim datawhere
      if Result="Search" then
	     datawhere="where VideoNameCh like '%" & Keyword &_
		           "%' "
	  else
	    if SortPath<>"" then
  ...
  if(idcount>0 and sqlid<>"") then
    ...
    while(not rs.eof)
	  ...
	  if StrLen((rs("VideoNameCh")))>40 then
        Response.Write "<td nowrap title='"&rs("VideoNameCh")&"' class=""forumRow""><a href='VideoEdit.asp?Result=Modify&ID="&rs("ID")&"''>"&StrLeft(rs("VideoNameCh"),37)&"</a></td>" & vbCrLf
      else
        Response.Write "<td nowrap title='"&rs("VideoNameCh")&"' class=""forumRow""><a href='VideoEdit.asp?Result=Modify&ID="&rs("ID")&"''>"&rs("VideoNameCh")&"</a></td>" & vbCrLf
      end if 
      ...
      Response.Write "<td align=""center""nowrap class=""forumRow""><a href='VideoEdit.asp?Result=Modify&ID="&rs("ID")&"'>修改</a></td>" & vbCrLf
 	  Response.Write "<td nowrap align='center' class=""forumRow""><input name='selectID' type='checkbox' value='"&rs("ID")&"'></td>" & vbCrLf
      Response.Write "</tr>" & vbCrLf
	  rs.movenext
    wend
    Response.Write "<tr>" & vbCrLf
    Response.Write "<td colspan='11' nowrap align=""right"" class=""forumRow""><input onClick=""CheckAll(this.form)"" name=""buttonAllSelect"" type=""button"" id=""submitAllSearch"" value=""全选""> <input onClick=""CheckOthers(this.form)"" name=""buttonOtherSelect"" type=""button"" id=""submitOtherSelect"" value=""反选""> <input name='batch' type='submit' value='删除所选' onClick=""return test();""></td>" & vbCrLf
    Response.Write "</tr>" & vbCrLf
  else
    response.write "<tr><td nowrap align='center' colspan='11' class=""forumRow"">暂无企业视频</td></tr>"
  end if
  Response.Write "<tr>" & vbCrLf
