$(document).ready(function() {
    ...
    if (firstLi.length > 0 && jQuery("#MenuMyDeskTopMore_Triangle").length <= 0) {
         firstLi.find("a").eq(0).append('<span id="MenuMyDeskTopMore_Triangle" style="background-image: url(AdminDefaultTheme/Images/seg_side.gif);"><img border="0" src="AdminDefaultTheme/Images/TopQuick01.gif"/></span>');
     }
...
function ShowHideLayer(ID) {
    if (ID != tID) {
        ...
        if (tID != "") {
            ...
            document.getElementById("A" + tID).style.backgroundImage = "url(AdminDefaultTheme/Images/digital_left.gif)";
            if (document.getElementById("A" + tID).childNodes.length < 2) {
                document.getElementById("Span" + tID).style.backgroundImage = "url(AdminDefaultTheme/Images/digital_side.gif)";
            }
             else {
        ...
        document.getElementById("A" + ID).style.backgroundImage = "url(AdminDefaultTheme/Images/seg_left.gif)";
        if (document.getElementById("A" + ID).childNodes.length < 2) {
            document.getElementById("Span" + ID).style.backgroundImage = "url(AdminDefaultTheme/Images/seg_side.gif)";
            if (tID != "" && document.getElementById("A" + tID).childNodes.length >= 2) {
                if (triangle) {
                     triangle.style.backgroundImage = "url(AdminDefaultTheme/Images/digital_side.gif)";
                 }
            }
        }
         else {
            ...
            if (triangle) {
                 triangle.style.backgroundImage = "url(AdminDefaultTheme/Images/seg_side.gif)";
             }
        }
