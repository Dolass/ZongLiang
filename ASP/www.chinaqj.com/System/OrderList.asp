...
function GetOrderList()
	...
	If request.QueryString("Result")="Search" and request.QueryString("Keyword")<>"" Then				'搜索关键字
		SqlWhere=" OrderName LIKE '%"&request.QueryString("Keyword")&"%'"	'where
	End If
	...
	Else
		Do While Not rs.eof
			...
			if StrLen(rs("OrderName"))>45 Then
				Response.Write "<td title="&rs("OrderName")&" nowrap class=""forumRow"">"&StrLeft(rs("OrderName"),42)&"</td>"
			Else
				Response.Write "<td title="&rs("OrderName")&" nowrap class=""forumRow"">"&rs("OrderName")&"</td>"
			end if 
			if StrLen(rs("Company"))>21 Then
				Response.Write "<td title="&rs("Company")&" nowrap class=""forumRow"">"&StrLeft(rs("Company"),18)&"</td>"
			Else
				Response.Write "<td title="&rs("Company")&" nowrap class=""forumRow"">"&rs("Company")&"</td>"
			end If
			...
			Response.Write "<td align=""center""nowrap class=""forumRow""><a href='OrderEdit.asp?Result=Modify&ID="&rs("ID")&"'>查看订单详情</a></td>"
