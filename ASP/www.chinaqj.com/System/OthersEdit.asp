...
dim ID,OthersNameCh,ViewFlagCh,OthersNameEn,ViewFlagEn,ClassSeo,SortName,SortID,SortPath
dim GroupID,GroupIdName,Exclusive,ContentCh,SeoKeywordsCh,SeoDescriptionCh,ContentEn,SeoKeywordsEn,SeoDescriptionEn
Dim hanzi,j,ChinaQJ,temp,temp1,flag,firstChar
dim Sequence,TitleColor,ChinaQJPic
...
<table class="tableBorder" width="95%" border="0" align="center" cellpadding="0" cellspacing="1">
  <form name="editForm" method="post" action="OthersEdit.asp?Action=SaveEdit&Result=<%=Result%>&ID=<%=ID%>">
    <tr>
      <th height="28" colspan="2" sytle="line-height:150%">【<%If Result = "Add" then%>添加<%ElseIf Result = "Modify" then%>修改<%End If%>信息】</th>
    </tr>
...
<table class="tableborderOther" width="100%" border="0" align="center" cellpadding="0" cellspacing="1">
    <tr height="35">
      ...
      <td width="80%" class="forumRowHighlight"><input name="OthersName<%= rs("ChinaQJ_Language_File") %>" type="text" id="OthersName<%= rs("ChinaQJ_Language_File") %>" style="width: 280" value="<%=eval("OthersName"&rs("ChinaQJ_Language_File"))%>" maxlength="250">
...
%>
  <script>showtabcontent("tablist")</script></td></tr>
    <tr>
      <td width="200" align="right" class="forumRow">图片：</td>
...
sub OthersEdit()
  ...
  if Action="SaveEdit" then
    set rs = server.createobject("adodb.recordset")
    if len(trim(request.Form("OthersNameCh")))<3 then
      response.write ("<script language='javascript'>alert('请填写信息标题！');history.back(-1);</script>")
      response.end
    end if
    if len(trim(request.Form("OthersNameEn")))<3 then
      response.write ("<script language='javascript'>alert('请填写信息标题！');history.back(-1);</script>")
      response.end
    end if
    ...
    if Result="Add" then
	  sql="select * from ChinaQJ_Others"
...
while(not rsl.eof)
  rs("OthersName"&rsl("ChinaQJ_Language_File"))=trim(Request.Form("OthersName"&rsl("ChinaQJ_Language_File")))
...
set rsl=nothing
	  If Request.Form("oAutopinyin") = "Yes" And Len(trim(Request.form("ClassSeo"))) = 0 Then
		rs("ClassSeo") = Left(Pinyin(trim(request.form("OthersNameCh"))),200)
	  Else
	  ...
	  set rs=Nothing
	  set rs=server.createobject("adodb.recordset")
	  sql="select top 1 ID,ClassSeo from ChinaQJ_Others order by ID desc"
	  rs.open sql,conn,1,1
	  ID=rs("ID")
...
'循环结束
	...
	if Result="Modify" then
      sql="select * from ChinaQJ_Others where ID="&ID
      rs.open sql,conn,1,3
  '多语言循环保存数据
set rsl = server.createobject("adodb.recordset")
sqll="select * from ChinaQJ_Language order by ChinaQJ_Language_Order"
...
set rsl=nothing
	  ...
	  set rs=Nothing
	  set rs=server.createobject("adodb.recordset")
	  sql="select top 1 ID,ClassSeo from ChinaQJ_Others where id="&ID
	  rs.open sql,conn,1,1
	  OtherNameSeo=rs("ClassSeo")
	  rs.close
	  set rs=Nothing
...
while(not rsl.eof)
  ...
  execute("OthersName"&Lanl&"=OthersName")
