...
<form action="DelContent.asp?Result=KeyIdea" method="post" name="formDel">
  <tr>
    <th width="8" align="left">ID</th>
    <th width="85%" align="left">创意标题</th>
    <th width="60">操作</th>
    <th width="60">选择</th>
  </tr>
  <% NewsList() %>
...
function NewsList()
  ...
  dim pagenmin
  dim datafrom
      datafrom="ChinaQJ_KeyIdea"
  dim datawhere
      if Result="Search" then
	     datawhere="where NewsNameCh like '%" & Keyword &_
		           "%' "
	  else
	    if SortPath<>"" then
  ...
  if(idcount>0 and sqlid<>"") then
    ...
    while(not rs.eof)
	  ...
	  if StrLen((rs("KeyIdeaNameCh")))>40 then
        Response.Write "<td nowrap title='"&rs("KeyIdeaNameCh")&"' class=""forumRow""><a href='KeyIdeaEdit.asp?Result=Modify&ID="&rs("ID")&"''>"&StrLeft(rs("KeyIdeaNameCh"),37)&"</a></td>" & vbCrLf
      else
        Response.Write "<td nowrap title='"&rs("KeyIdeaNameCh")&"' class=""forumRow""><a href='KeyIdeaEdit.asp?Result=Modify&ID="&rs("ID")&"''>"&rs("KeyIdeaNameCh")&"</a></td>" & vbCrLf
      end if 
      Response.Write "<td align=""center""nowrap class=""forumRow""><a href='KeyIdeaEdit.asp?Result=Modify&ID="&rs("ID")&"'>修改</a></td>" & vbCrLf
    ...
    response.write "<tr><td nowrap align='center' colspan='11' class=""forumRow"">暂无创意内容</td></tr>"
  end if
  Response.Write "<tr>" & vbCrLf
