...
Select Case Action
  ...
  Case "Del"
    ...
    sql="Select * From ChinaQJ_NewsSort where ID="&request.QueryString("id")
    rs.open sql,conn,1,1
	SortPath=rs("SortPath")
	conn.execute("delete from ChinaQJ_NewsSort where Instr(SortPath,'"&SortPath&"')>0")
    conn.execute("delete from ChinaQJ_News where Instr(SortPath,'"&SortPath&"')>0")
...
Function Folder(id)
  ...
  Set rs=server.CreateObject("adodb.recordset")
  sql="Select * From ChinaQJ_NewsSort where ParentID="&id&" order by Sequence"
  rs.open sql,conn,1,1
...
set rs2=nothing
    ...
    response.write(" | <a href='NewsSort.asp?Action=Move&ID="&rs("id")&"&ParentID="&rs("Parentid")&"&SortName="&rs("SortNameCh")&"&SortPath="&rs("SortPath")&"'>移</a>")
...
Function addFolderForm(ParentID)
  ...
  if ParentID=0 then
    ...
    Set rs=server.CreateObject("adodb.recordset")
    sql="Select * From ChinaQJ_NewsSort where ID="&ParentID
    rs.open sql,conn,1,1
...
Function saveFolder
  ...
  Set rs=server.CreateObject("adodb.recordset")
  if From="Add" then
    sql="Select * From ChinaQJ_NewsSort"
...
Function saveMoveFolder()
  ...
  Set rs=server.CreateObject("adodb.recordset")
  sql="Select * From ChinaQJ_NewsSort where ID="&fromParentID
  rs.open sql,conn,0,1
  fromParentSortPath=rs("SortPath")
  conn.execute("update ChinaQJ_NewsSort set SortPath='"&toSortPath&"'+Mid(SortPath,Len('"&fromParentSortPath&"')+1) where Instr(SortPath,'"&fromSortPath&"')>0")
  conn.execute("update ChinaQJ_NewsSort set ParentID='"&toID&"' where ID="&fromID)
  conn.execute("update ChinaQJ_News set SortPath='"&toSortPath&"'+Mid(SortPath,Len('"&fromParentSortPath&"')+1) where Instr(SortPath,'"&fromSortPath&"')>0")
