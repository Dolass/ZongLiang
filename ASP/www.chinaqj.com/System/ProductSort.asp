...
Select Case Action
  ...
  Case "Del"
    ...
    sql="Select * From ChinaQJ_ProductSort where ID="&request.QueryString("id")
    rs.open sql,conn,1,1
	SortPath=rs("SortPath")
	conn.execute("delete from ChinaQJ_ProductSort where Instr(SortPath,'"&SortPath&"')>0")
    conn.execute("delete from ChinaQJ_Products where Instr(SortPath,'"&SortPath&"')>0")
...
Function Folder(id)
  ...
  Set rs=server.CreateObject("adodb.recordset")
  sql="Select * From ChinaQJ_ProductSort where ParentID="&id&" order by Sequence"
  rs.open sql,conn,1,1
...
set rs2=nothing
    ...
    response.write(" | <a href='ProductSort.asp?Action=Move&ID="&rs("id")&"&ParentID="&rs("Parentid")&"&SortName="&rs("SortNameCh")&"&SortPath="&rs("SortPath")&"'>移</a>")
...
Function addFolderForm(ParentID)
  ...
  if ParentID=0 then
    ...
    Set rs=server.CreateObject("adodb.recordset")
    sql="Select * From ChinaQJ_ProductSort where ID="&ParentID
    rs.open sql,conn,1,1
...
<table class="tableBorder" width="95%" border="0" align="center" cellpadding="0" cellspacing="1">
  <form name="FolderForm" method="post" action="ProductSort.asp?Action=Save&From=Add">
    <tr>
      <th height="22" sytle="line-height:150%" colspan="2">【添加产品类别】</th>
    </tr>
    <tr height="35">
...
Function saveFolder
  ...
  Set rs=server.CreateObject("adodb.recordset")
  if From="Add" then
    sql="Select * From ChinaQJ_ProductSort"
...
<table class="tableBorder" width="95%" border="0" align="center" cellpadding="0" cellspacing="1">
  <form name="FolderForm" method="post" action="ProductSort.asp?Action=Save&From=Edit&ID=<%=ID%>">
    <tr>
      <th height="22" sytle="line-height:150%" colspan="2">【修改产品类别】</th>
    </tr>
    <tr height="35">
      <td class="forumRow" colspan="2">| 根类 →
        <% if ParentID<>0 then TextPath(ParentID)%></td>
...
Function saveMoveFolder()
  ...
  Set rs=server.CreateObject("adodb.recordset")
  sql="Select * From ChinaQJ_ProductSort where ID="&fromParentID
  rs.open sql,conn,0,1
  fromParentSortPath=rs("SortPath")
  conn.execute("update ChinaQJ_ProductSort set SortPath='"&toSortPath&"'+Mid(SortPath,Len('"&fromParentSortPath&"')+1) where Instr(SortPath,'"&fromSortPath&"')>0")
  conn.execute("update ChinaQJ_ProductSort set ParentID='"&toID&"' where ID="&fromID)
  conn.execute("update ChinaQJ_Products set SortPath='"&toSortPath&"'+Mid(SortPath,Len('"&fromParentSortPath&"')+1) where Instr(SortPath,'"&fromSortPath&"')>0")
