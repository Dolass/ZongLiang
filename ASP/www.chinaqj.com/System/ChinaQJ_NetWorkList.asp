...
<form action="DelContent.Asp?Result=NetWork" method="post" name="formDel">
  <tr>
    <th width="8">ID</th>
    <th align="left">销售点</th>
    <th align="left">地址</th>
    <th align="left" width="120">联系人</th>
...
<td nowrap class="leftrow"><font color="red">【<%= rs("ChinaQJ_Province") %>】</font><%= rs("ChinaQJ_CompanyCh") %></td>
<td nowrap class="leftrow"><%= rs("ChinaQJ_AddressCh") %></td>
<td nowrap class="leftrow"><%= rs("ChinaQJ_UserCh") %></td>
<td nowrap class="leftrow"><%= rs("ChinaQJ_AddTime") %></td>
<td nowrap class="centerrow"><a href='ChinaQJ_NetWorkEdit.Asp?Result=Modify&ID=<%= rs("id") %>&ChinaQJ_Province=<%= rs("ChinaQJ_Province") %>'>修改</a></td>
