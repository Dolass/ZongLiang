...
Const nNoisyLineCount =4				' 噪音线条数
Const nCharMin = 4							' 产生的最小字符个数
Const nCharMax = 4							' 产生的最大字符个数
Const nSpaceX = 2								' 左右两边的空白宽度
Const nSpaceY = 2								' 上下两边的空白宽度
IF Isnumeric(Request.QueryString("t0")) And Request.QueryString("t0")<>"" Then nImgWidth =Request.QueryString("t0")	Else nImgWidth=64			' 数据区宽度
IF Isnumeric(Request.QueryString("t1")) And Request.QueryString("t1")<>"" Then nImgHeight=Request.QueryString("t1")	Else nImgHeight=18			' 数据区高度				
