...
(function () {
    ...
    var tmp = window.location.pathname,
        URL = this_path + "ueditor/"; //window.UEDITOR_HOME_URL||tmp.substr(0,tmp.lastIndexOf("\/")+1).replace("_examples/","").replace("website/","");//这里你可以配置成ueditor目录在您网站的相对路径或者绝对路径（指以http开头的绝对路径）
    UEDITOR_CONFIG = {
        ...
        filePath:"/upload/file/",  //附件文件夹保存路径
	    catchRemoteImageEnable:true,                                   //是否开启远程图片抓取
        catcherUrl:URL + "server/" + this_ext + "/getRemoteImage." + this_ext,             //处理远程图片抓取的地址
        localDomain:"baidu.com",                                        //本地顶级域名，当开启远程图片抓取时，除此之外的所有其它域名下的图片都将被抓取到本地
	    imageManagerPath:URL + "server/" + this_ext + "/imageManager." + this_ext,       //图片在线浏览的处理地址
        UEDITOR_HOME_URL:URL,                                          //为editor添加一个全局路径
		MY_ROOT_PATH:this_root,
