...
function PlaceFlag()
  if Result="Search" Then
    ...
    if SortPath<>"" then
      Response.Write "下载：列表 -> <a href='MagazineList.asp'>全部</a>"
	  TextPath(SortID)
	else
...
<form name="formSearch" method="post" action="Search.asp?Result=Magazine">
  <tr>
...
<%
	'Call BetaNewSo("MagazineList.asp","","NewFlagCh","新品")
	'Call BetaNewSo("MagazineList.asp","","CommendFlagCh","推荐")
	Call BetaNewSo("MagazineList.asp","NOT ","ViewFlagCh","中文未显示")
	Call BetaNewSo("MagazineList.asp","NOT ","ViewFlagEn","英文未显示")
	Call BetaNewSo("MagazineList.asp","NOT ","ViewFlagJp","日文未显示")
%>
  </td></tr>
  </form>
</table>
<br />
<table class="tableBorder" width="95%" border="0" align="center" cellpadding="5" cellspacing="1">
<form action="DelContent.asp?Result=Magazine" method="post" name="formDel">
  <tr>
      <th>ID</th>
<% 
...
set rs=nothing
%>
      <th width="70%" align="left">电子杂志标题</th>
      <th width="5%">排序</th>
      <th width="10%">发布时间</th>
      <th width="5%">操作</th>
      <th width="5%">选择</th>
...
function GetMagazineList()
	...
	If request.QueryString("Result")="Search" and request.QueryString("Keyword")<>"" Then				'搜索关键字
		SqlWhere=" MagazineNameCh LIKE '%"&request.QueryString("Keyword")&"%'"	'where
	End If
	...
	Else
		Do While Not rs.eof
			...
			if StrLen((rs("MagazineNameCh")))>40 Then
				Response.Write "<td nowrap title='"&rs("MagazineNameCh")&"' class=""forumRow"" width=""50%""><a href='MagazineEdit.asp?Result=Modify&ID="&rs("ID")&"''>"&StrLeft(rs("MagazineName"),37)&"</a></td>"
			Else
				Response.Write "<td nowrap title='"&rs("MagazineNameCh")&"' class=""forumRow"" width=""50%""><a href='MagazineEdit.asp?Result=Modify&ID="&rs("ID")&"''>"&rs("MagazineNameCh")&"</a></td>"
			end if 
			Response.Write "<td nowrap class=""forumRow"">"&rs("Sequence")&"</td>"
			Response.Write "<td nowrap class=""forumRow"">"&rs("AddTime")&"</td>"
			Response.Write "<td align=""center""nowrap class=""forumRow""><a href='MagazineEdit.asp?Result=Modify&ID="&rs("ID")&"'>修改</a></td>"
...
function MagazineList()
  ...
  dim datawhere
      if Result="Search" then
	     datawhere="where MagazineNameCh like '%" & Keyword &_
		           "%' "
	  else
	    if SortPath<>"" then
...
while(not rs2.eof)
      ...
      if rs("ViewFlag"&rs2("ChinaQJ_Language_File")) then
        Response.Write "<td nowrap align=""center"" class=""forumRow""><a href=""Conversion.asp?id="&rs("ID")&"&LX="&datafrom&"&Operation=Magazine&ViewLanguage="&rs2("ChinaQJ_Language_File")&"""><font color='blue'>√</font></a></td>" & vbCrLf
      else
...
set rs2=nothing
	  if StrLen((rs("MagazineNameCh")))>40 then
        Response.Write "<td nowrap title='"&rs("MagazineNameCh")&"' class=""forumRow"" width=""50%""><a href='MagazineEdit.asp?Result=Modify&ID="&rs("ID")&"''>"&StrLeft(rs("MagazineName"),37)&"</a></td>" & vbCrLf
      else
        Response.Write "<td nowrap title='"&rs("MagazineNameCh")&"' class=""forumRow"" width=""50%""><a href='MagazineEdit.asp?Result=Modify&ID="&rs("ID")&"''>"&rs("MagazineNameCh")&"</a></td>" & vbCrLf
      end if 
	  Response.Write "<td nowrap class=""forumRow"">"&rs("Sequence")&"</td>" & vbCrLf
	  Response.Write "<td nowrap class=""forumRow"">"&rs("AddTime")&"</td>" & vbCrLf
      Response.Write "<td align=""center""nowrap class=""forumRow""><a href='MagazineEdit.asp?Result=Modify&ID="&rs("ID")&"'>修改</a></td>" & vbCrLf
 	  Response.Write "<td nowrap align='center' class=""forumRow""><input name='selectID' type='checkbox' value='"&rs("ID")&"'></td>" & vbCrLf
      Response.Write "</tr>" & vbCrLf
	  rs.movenext
    wend
    Response.Write "<tr>" & vbCrLf
    Response.Write "<td colspan='99' nowrap align=""right"" class=""forumRow""><input onClick=""CheckAll(this.form)"" name=""buttonAllSelect"" type=""button"" id=""submitAllSearch"" value=""全选""> <input onClick=""CheckOthers(this.form)"" name=""buttonOtherSelect"" type=""button"" id=""submitOtherSelect"" value=""反选""> <input name='batch' type='submit' value='删除所选' onClick=""return test();""></td>" & vbCrLf
    Response.Write "</tr>" & vbCrLf
  else
    response.write "<tr><td nowrap align='center' colspan='99' class=""forumRow"">暂无相关信息！</td></tr>"
  end if
  Response.Write "<tr>" & vbCrLf
  ...
  Response.Write "第<input name='SkipPage' onKeyMagazine='if(event.keyCode==13)event.returnValue=false' onchange=""if(/\D/.test(this.value)){alert('请输入需要跳转到的页数并且必须为整数！');this.value='"&Page&"';}"" style='width: 28px;' type='text' value='"&Page&"'>页" & vbCrLf
...
Function TextPath(ID)
  ...
  rs.open sql,conn,1,1
  TextPath=" -> <a href=""MagazineList.asp?SortID="&rs("ID")&"&SortPath="&rs("SortPath")&""">"&rs("SortNameCh")&"</a>"
