...
dim ID,ImageNameCh,ImageNameEn,ClassSeo,ViewFlagCh,ViewFlagEn,SortName,SortID,SortPath,TitleColor,ChinaQJPic,Sequence,ImageFile
...
<table class="tableBorder" width="95%" border="0" align="center" cellpadding="0" cellspacing="1">
  <form name="editForm" method="post" action="ImageEdit.asp?Action=SaveEdit&Result=<%=Result%>&ID=<%=ID%>">
    <tr>
...
<table class="tableborderOther" width="100%" border="0" align="center" cellpadding="0" cellspacing="1">
    <tr height="35">
      ...
      <td width="80%" class="forumRowHighlight"><input name="ImageName<%= rs("ChinaQJ_Language_File") %>" type="text" id="ImageName<%= rs("ChinaQJ_Language_File") %>" style="width: 280" value="<%=eval("ImageName"&rs("ChinaQJ_Language_File"))%>" maxlength="100">
...
%>
  <script>showtabcontent("tablist")</script></td></tr>
    ...
    <tr height="28">
      <td align="right" class="forumRow">小图地址：</td>
      <td class="forumRowHighlight"><input name="ChinaQJPic" type="text" id="ChinaQJPic" style="width: 300px;" value="<%= ChinaQJPic %>" maxlength="100"> <input type="button" value="上传图片" onclick="showUploadDialog('image', 'editForm.ChinaQJPic', '')"> 
      为了显示效果，请将图片大小保持为101*61像素。</td>
    </tr>
    <tr height="35">
      <td align="right" class="forumRow">大图地址：</td>
      <td class="forumRowHighlight"><input name="ImageFile" type="text" id="ImageFile" style="width: 300" value="<%= ImageFile %>" maxlength="100"> <input type="button" value="上传图片" onclick="showUploadDialog('image', 'editForm.ImageFile', '')"> 为了显示效果，请将图片大小保持为582*340像素。</td>
    </tr>
    <tr height="35">
      <td align="right" class="forumRow">所属类别：</td>
      <td class="forumRowHighlight"><input name="SortID" type="text" id="SortID" style="width: 18; background-color:#fffff0" value="<%=SortID%>" readonly> <input name="SortPath" type="text" id="SortPath" style="width: 70; background-color:#fffff0" value="<%=SortPath%>" readonly> <input name="SortName" type="text" id="SortName" value="<%=SortName%>" style="width: 180; background-color:#fffff0" readonly> <a href="javaScript:OpenScript('SelectSort.asp?Result=Image',500,500,'')">选择类别</a> <font color="red">*</font></td>
    </tr>
    <tr height="35">
      <td align="right" class="forumRow">自定义发布时间：</td>
      <td class="forumRowHighlight"><input name="AddTime" type="text" id="AddTime" style="width: 120px;" value="<%= now() %>" onFocus="WdatePicker({startDate:'%y-%M-%d  %H:%m:%s',dateFmt:'yyyy-MM-dd HH:mm:ss',alwaysUseStartDate:true})" readonly> <font color="red">默认为当前时间，如需要修改发布时间，请点击文本框选择新时间。留空默认为当前时间。</font></td>
    </tr>
    <tr height="35">
...
sub ImageEdit()
  ...
  if Action="SaveEdit" then
    set rs = server.createobject("adodb.recordset")
    if len(trim(request.Form("ImageNameCh")))<3 then
...
while(not rsl.eof)
  rs("ImageName"&rsl("ChinaQJ_Language_File"))=trim(Request.Form("ImageName"&rsl("ChinaQJ_Language_File")))
...
set rsl=nothing
	  ...
	  rs("ImageFile")=trim(Request.Form("ImageFile"))
	  ...
	  If Request.Form("oAutopinyin") = "Yes" And Len(trim(Request.form("ClassSeo"))) = 0 Then
		rs("ClassSeo") = Left(Pinyin(trim(request.form("ImageNameCh"))),200)
	  Else
	  ...
	  set rs=Nothing
	  set rs=server.createobject("adodb.recordset")
	  sql="select top 1 ID,ClassSeo from ChinaQJ_Image order by ID desc"
	  rs.open sql,conn,1,1
	  ID=rs("ID")
...
call htmll("","",""&LanguageFolder&""&NewNameSeo&""&Separated&""&ID&"."&HTMLName&"",""&LanguageFolder&"ImageView.asp","ID=",ID,"","")
rsh.movenext
wend
rsh.close
set rsh=nothing
'循环结束
	...
	if Result="Modify" then
      sql="select * from ChinaQJ_Image where ID="&ID
      rs.open sql,conn,1,3
  '多语言循环保存数据
set rsl = server.createobject("adodb.recordset")
sqll="select * from ChinaQJ_Language order by ChinaQJ_Language_Order"
...
set rsl=nothing
	  ...
	  set rs=Nothing
	  set rs=server.createobject("adodb.recordset")
	  sql="select top 1 ID,ClassSeo from ChinaQJ_Image where id="&ID
	  rs.open sql,conn,1,1
	  NewNameSeo=rs("ClassSeo")
	  rs.close
	  set rs=Nothing
...
'循环结束
    ...
    if ISHTML = 1 then
    response.write "<script language='javascript'>alert('设置成功，相关静态页面已更新！');location.replace('ImageList.asp');</script>"
	Else
	response.write "<script language='javascript'>alert('设置成功！');location.replace('ImageList.asp');</script>"
	End If
  else
	if Result="Modify" then
      set rs = server.createobject("adodb.recordset")
...
while(not rsl.eof)
  ...
  execute("ImageName"&Lanl&"=ImagesName")
