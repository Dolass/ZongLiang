...
function PlaceFlag()
  if Result="Search" Then
	If Keyword<>"" Then
		Response.Write "长尾关键字：列表 -> 检索 -> 关键字：<font color='red'>"&Keyword&"</font>"
	Else
		Response.Write "长尾关键字：列表 -> 检索 -> 关键字为空(显示全部长尾关键字)"
	End If
  else
    if SortPath<>"" then
      Response.Write "长尾关键字：列表 -> <a href='KeyList.asp'>全部</a>"
	  TextPath(SortID)
	else
      Response.Write "长尾关键字：列表 -> 全部"
	end if
  end if
end function
%>
...
<form name="formSearch" method="post" action="Search.asp?Result=Key">
  <tr>
    <th height="22" sytle="line-height:150%">【长尾关键字检索及分类查看】</th>
  </tr>
  <tr>
    <td class="forumRow">关键字：<input name="Keyword" type="text" value="<%=Keyword%>" size="20"> <input name="submitSearch" type="submit" value="搜索长尾关键字"></td>
  </tr>
  <tr>
...
<br />
<table class="tableBorder" width="95%" border="0" align="center" cellpadding="5" cellspacing="1">
<form action="DelContent.asp?Result=Key" method="post" name="formDel">
  <tr>
    <th width="8">ID</th>
	<th width="90%">长尾关键字标题</th>
	<th width="60">选择</th>
  </tr>
  <% NewsList() %>
  </form>
...
function NewsList()
  ...
  dim datawhere
      if Result="Search" then
	     datawhere="where Content like '%" & Keyword &_
		           "%' "
	  else
	    if SortPath<>"" then
  ...
  if(idcount>0 and sqlid<>"") then
    ...
    while(not rs.eof)
	  ...
	  if StrLen((rs("Content")))>40 then
        Response.Write "<td nowrap title='"&rs("Content")&"' class=""forumRow"">"&StrLeft(rs("Content"),37)&"</td>" & vbCrLf
      else
        Response.Write "<td nowrap title='"&rs("Content")&"' class=""forumRow"">"&rs("Content")&"</td>" & vbCrLf
      end if 
    ...
    response.write "<tr><td nowrap align='center' colspan='11' class=""forumRow"">暂无企业长尾关键字</td></tr>"
  end if
  Response.Write "<tr>" & vbCrLf
...
Function TextPath(ID)
  ...
  rs.open sql,conn,1,1
  TextPath=" -> <a href=""NewsList.asp?SortID="&rs("ID")&"&SortPath="&rs("SortPath")&""">"&rs("SortNameCh")&"</a>"
