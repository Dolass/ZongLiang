...
Select Case Action
  ...
  Case "Del"
    ...
    sql="Select * From ChinaQJ_VideoSort where ID="&request.QueryString("id")
    rs.open sql,conn,1,1
	SortPath=rs("SortPath")
	conn.execute("delete from ChinaQJ_VideoSort where Instr(SortPath,'"&SortPath&"')>0")
    conn.execute("delete from ChinaQJ_Video where Instr(SortPath,'"&SortPath&"')>0")
    response.write ("<script language='javascript'>alert('成功删除本类、子类及所有下属信息条目！');location.replace('VideoSort.asp');</script>")
  Case "Save"
	saveFolder ()
  Case "Edit"
	editFolder
...
<table class="tableBorder" width="95%" border="0" align="center" cellpadding="0" cellspacing="1">
  <tr>
    <th height="22" sytle="line-height:150%">【管理视频类别】</th>
  </tr>
  <tr>
    <td align="center" nowrap class="forumRow"><a href="VideoSort.asp?Action=Add&ParentID=0">添加一级分类</a> | <a href="ImageList.asp">查看所有视频</a></td>
  </tr>
  <tr>
...
Function Folder(id)
  ...
  Set rs=server.CreateObject("adodb.recordset")
  sql="Select * From ChinaQJ_VideoSort where ParentID="&id&" order by Sequence"
  rs.open sql,conn,1,1
  if id=0 and rs.recordcount=0 then
    response.write ("<center>暂无视频分类</center>")
    response.end
  end if
  i=1
  response.write("<table border='0' cellspacing='0' cellpadding='0'>")
  while not rs.eof
    ChildCount=conn.execute("select count(*) from ChinaQJ_VideoSort where ParentID="&rs("id"))(0)
...
rs2.close
set rs2=nothing
    response.write("&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<font color='red'>操作：</font><a href='VideoSort.asp?Action=Add&ParentID="&rs("id")&"'>添加</a>")
    response.write(" | <a href='VideoSort.asp?Action=Edit&ID="&rs("id")&"'>修改</a>")
    response.write(" | <a href='VideoSort.asp?Action=Move&ID="&rs("id")&"&ParentID="&rs("Parentid")&"&SortName="&rs("SortNameCh")&"&SortPath="&rs("SortPath")&"'>移</a>")
    ...
    response.write("&nbsp;&nbsp;&nbsp;&nbsp;<font color='red'>视频：</font><a href='ImageEdit.asp?Result=Add'>添加</a>")
...
Function addFolderForm(ParentID)
  ...
  if ParentID=0 then
    ...
    Set rs=server.CreateObject("adodb.recordset")
    sql="Select * From ChinaQJ_VideoSort where ID="&ParentID
    rs.open sql,conn,1,1
...
<br />
<table class="tableBorder" width="95%" border="0" align="center" cellpadding="0" cellspacing="1">
  <form name="FolderForm" method="post" action="VideoSort.asp?Action=Save&From=Add">
    <tr>
      <th height="22" sytle="line-height:150%" colspan="2">【添加视频类别】</th>
    </tr>
    <tr height="35">
...
Function saveFolder
  ...
  Set rs=server.CreateObject("adodb.recordset")
  if From="Add" then
    sql="Select * From ChinaQJ_VideoSort"
...
set rsl=nothing
  ...
  rs.Update
  response.write ("<script language='javascript'>alert('"&Action&"成功！');location.replace('VideoSort.asp');</script>")
End Function
...
<br />
<table class="tableBorder" width="95%" border="0" align="center" cellpadding="0" cellspacing="1">
  <form name="FolderForm" method="post" action="VideoSort.asp?Action=Save&From=Edit&ID=<%=ID%>">
    <tr>
      <th height="22" sytle="line-height:150%" colspan="2">【修改视频类别】</th>
    </tr>
    <tr height="35">
      <td class="forumRow" colspan="2">| 根类 →
        <% if ParentID<>0 then TextPath(ParentID)%></td>
...
<table id="SortFromTo" class="tableBorder" width="95%" border="0" align="center" cellpadding="0" cellspacing="1">
  <form name="MoveForm" method="post" action="VideoSort.asp?Action=MoveSave">
    <tr>
      <th height="22" sytle="line-height:150%">【移动视频类别】</th>
    </tr>
    <tr>
      <td class="forumRow">→
...
Function saveMoveFolder()
  ...
  Set rs=server.CreateObject("adodb.recordset")
  sql="Select * From ChinaQJ_VideoSort where ID="&fromParentID
  rs.open sql,conn,0,1
  fromParentSortPath=rs("SortPath")
  conn.execute("update ChinaQJ_VideoSort set SortPath='"&toSortPath&"'+Mid(SortPath,Len('"&fromParentSortPath&"')+1) where Instr(SortPath,'"&fromSortPath&"')>0")
  conn.execute("update ChinaQJ_VideoSort set ParentID='"&toID&"' where ID="&fromID)
  conn.execute("update ChinaQJ_Video set SortPath='"&toSortPath&"'+Mid(SortPath,Len('"&fromParentSortPath&"')+1) where Instr(SortPath,'"&fromSortPath&"')>0")
  response.write ("<script language='javascript'>alert('视频类别移动成功！');location.replace('VideoSort.asp');</script>")
End Function
%>