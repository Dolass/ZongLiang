...
Select Case Action
  ...
  Case "Del"
    ...
    sql="Select * From ChinaQJ_MagazineSort where ID="&request.QueryString("id")
    rs.open sql,conn,1,1
	SortPath=rs("SortPath")
	conn.execute("delete from  ChinaQJ_MagazineSort where Instr(SortPath,'"&SortPath&"')>0")
    conn.execute("delete from  ChinaQJ_Magazineload where Instr(SortPath,'"&SortPath&"')>0")
    response.write ("<script language='javascript'>alert('成功删除本类、子类及所有下属信息条目！');location.replace('MagazineSort.asp');</script>")
  Case "Save"
	saveFolder ()
  Case "Edit"
	editFolder
...
<table class="tableBorder" width="95%" border="0" align="center" cellpadding="0" cellspacing="1">
  <tr>
    <th height="22" sytle="line-height:150%">【管理电子杂志类别】</th>
  </tr>
  <tr>
    <td align="center" nowrap class="forumRow"><a href="MagazineSort.asp?Action=Add&ParentID=0">添加一级分类</a> | <a href="MagazineList.asp">查看所有电子杂志</a></td>
  </tr>
  <tr>
...
Function Folder(id)
  ...
  Set rs=server.CreateObject("adodb.recordset")
  sql="Select * From ChinaQJ_MagazineSort where ParentID="&id&" order by Sequence"
  rs.open sql,conn,1,1
  if id=0 and rs.recordcount=0 then
    response.write ("<center>暂无电子杂志分类</center>")
    response.end
  end if
  i=1
  response.write("<table border='0' cellspacing='0' cellpadding='0'>")
  while not rs.eof
    ChildCount=conn.execute("select count(*) from ChinaQJ_MagazineSort where ParentID="&rs("id"))(0)
...
while(not rs2.eof)
      ...
      if rs("ViewFlag"&rs2("ChinaQJ_Language_File")) then
        Response.Write "<a href=""Conversion.asp?id="&rs("ID")&"&LX="&datafrom&"&Operation=Magazine&ViewLanguage="&rs2("ChinaQJ_Language_File")&"""><font color='blue'>√</font></a>" & vbCrLf
      else
...
rs2.close
set rs2=nothing
    response.write("&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<font color='red'>操作：</font><a href='MagazineSort.asp?Action=Add&ParentID="&rs("id")&"'>添加</a>")
    response.write(" | <a href='MagazineSort.asp?Action=Edit&ID="&rs("id")&"'>修改</a>")
    response.write(" | <a href='MagazineSort.asp?Action=Move&ID="&rs("id")&"&ParentID="&rs("Parentid")&"&SortName="&rs("SortNameCh")&"&SortPath="&rs("SortPath")&"'>移</a>")
	...
	response.write(" | <a href=javascript:ConfirmDelSort('MagazineSort',"&rs("id")&")>删除</a>")
    response.write("&nbsp;&nbsp;&nbsp;&nbsp;<font color='red'>电子杂志：</font><a href='MagazineEdit.asp?Result=Add'>添加</a>")
    response.write(" | <a href='MagazineList.asp?SortID="&rs("ID")&"&SortPath="&rs("SortPath")&"'>列表</a>")
...
Function addFolderForm(ParentID)
  ...
  if ParentID=0 then
    ...
    Set rs=server.CreateObject("adodb.recordset")
    sql="Select * From ChinaQJ_MagazineSort where ID="&ParentID
    rs.open sql,conn,1,1
...
<br />
<table class="tableBorder" width="95%" border="0" align="center" cellpadding="0" cellspacing="1">
  <form name="FolderForm" method="post" action="MagazineSort.asp?Action=Save&From=Add">
    <tr>
      <th height="22" sytle="line-height:150%" colspan="2">【添加电子杂志类别】</th>
    </tr>
    <tr height="35">
...
Function saveFolder
  ...
  Set rs=server.CreateObject("adodb.recordset")
  if From="Add" then
    sql="Select * From ChinaQJ_MagazineSort"
...
set rsl=nothing
  ...
  If Request.Form("oAutopinyin") = "Yes" And Len(trim(Request.form("ClassSeo"))) = 0 Then
	rs("ClassSeo") = Left(Pinyin(trim(request.form("SortNameCh"))),200)
  Else
  ...
  rs.update
  response.write ("<script language='javascript'>alert('"&Action&"成功！');location.replace('MagazineSort.asp');</script>")
End Function
...
<br />
<table class="tableBorder" width="95%" border="0" align="center" cellpadding="0" cellspacing="1">
  <form name="FolderForm" method="post" action="MagazineSort.asp?Action=Save&From=Edit&ID=<%=ID%>">
    <tr>
      <th height="22" sytle="line-height:150%" colspan="2">【修改电子杂志类别】</th>
    </tr>
    <tr height="35">
      <td class="forumRow" colspan="2">| 根类 →
        <% if ParentID<>0 then TextPath(ParentID)%></td>
...
<table id="SortFromTo" class="tableBorder" width="95%" border="0" align="center" cellpadding="0" cellspacing="1">
  <form name="MoveForm" method="post" action="MagazineSort.asp?Action=MoveSave">
    <tr>
      <th height="22" sytle="line-height:150%">【移动电子杂志类别】</th>
    </tr>
    <tr>
      <td class="forumRow">→
...
Function saveMoveFolder()
  ...
  Set rs=server.CreateObject("adodb.recordset")
  sql="Select * From ChinaQJ_MagazineSort where ID="&fromParentID
  rs.open sql,conn,0,1
  fromParentSortPath=rs("SortPath")
  conn.execute("update ChinaQJ_MagazineSort set SortPath='"&toSortPath&"'+Mid(SortPath,Len('"&fromParentSortPath&"')+1) where Instr(SortPath,'"&fromSortPath&"')>0")
  conn.execute("update ChinaQJ_MagazineSort set ParentID='"&toID&"' where ID="&fromID)
  conn.execute("update ChinaQJ_Magazineload set SortPath='"&toSortPath&"'+Mid(SortPath,Len('"&fromParentSortPath&"')+1) where Instr(SortPath,'"&fromSortPath&"')>0")
  response.write ("<script language='javascript'>alert('电子杂志类别移动成功！');location.replace('MagazineSort.asp');</script>")
End Function
%>