...
function PlaceFlag()
  if Result="Search" Then
    ...
    if SortPath<>"" then
      Response.Write "新闻：列表 -> <a href='ImageList.asp'>全部</a>"
	  TextPath(SortID)
	else
...
<form name="formSearch" method="post" action="Search.asp?Result=Image">
  <tr>
...
<%
	'Call BetaNewSo("ImageList.asp","","NewFlagCh","新品")
	'Call BetaNewSo("ImageList.asp","","CommendFlagCh","推荐")
	Call BetaNewSo("ImageList.asp","NOT ","ViewFlagCh","中文未显示")
	Call BetaNewSo("ImageList.asp","NOT ","ViewFlagEn","英文未显示")
	Call BetaNewSo("ImageList.asp","NOT ","ViewFlagJp","日文未显示")
%>
  </td></tr>
  </form>
</table>
<br />
<table class="tableBorder" width="95%" border="0" align="center" cellpadding="5" cellspacing="1">
<form action="DelContent.asp?Result=Image" method="post" name="formDel">
  <tr>
    <th>ID</th>
<% 
...
function GetImageList()
	...
	If request.QueryString("Result")="Search" and request.QueryString("Keyword")<>"" Then				'搜索关键字
		SqlWhere=" ImageNameCh LIKE '%"&request.QueryString("Keyword")&"%'"	'where
	End If
	...
	Else
		Do While Not rs.eof
			...
			if StrLen((rs("ImageNameCh")))>40 Then
				Response.Write "<td nowrap title='"&rs("ImageNameCh")&"' class=""forumRow""><a href='ImageEdit.asp?Result=Modify&ID="&rs("ID")&"''>"&StrLeft(rs("ImageNameCh"),37)&"</a></td>"
			Else
				Response.Write "<td nowrap title='"&rs("ImageNameCh")&"' class=""forumRow""><a href='ImageEdit.asp?Result=Modify&ID="&rs("ID")&"''>"&rs("ImageNameCh")&"</a></td>"
			end if 
			Response.Write "<td nowrap class=""forumRow"">"&rs("AddTime")&"</td>"
			Response.Write "<td nowrap class=""forumRow"">"&rs("ClickNumber")&"</td>"
			Response.Write "<td nowrap class=""forumRow"">"&rs("Sequence")&"</td>"
			Response.Write "<td align=""center""nowrap class=""forumRow""><a href='ImageEdit.asp?Result=Modify&ID="&rs("ID")&"'>修改</a></td>"
...
function ImageList()
  ...
  dim datawhere
      if Result="Search" then
	     datawhere="where ImageNameCh like '%" & Keyword &_
		           "%' "
	  else
	    if SortPath<>"" then
...
set rs2=nothing
	  if StrLen((rs("ImageNameCh")))>40 then
        Response.Write "<td nowrap title='"&rs("ImageNameCh")&"' class=""forumRow""><a href='ImageEdit.asp?Result=Modify&ID="&rs("ID")&"''>"&StrLeft(rs("ImageNameCh"),37)&"</a></td>" & vbCrLf
      else
        Response.Write "<td nowrap title='"&rs("ImageNameCh")&"' class=""forumRow""><a href='ImageEdit.asp?Result=Modify&ID="&rs("ID")&"''>"&rs("ImageNameCh")&"</a></td>" & vbCrLf
      end if 
      ...
      Response.Write "<td align=""center""nowrap class=""forumRow""><a href='ImageEdit.asp?Result=Modify&ID="&rs("ID")&"'>修改</a></td>" & vbCrLf
 	  Response.Write "<td nowrap align='center' class=""forumRow""><input name='selectID' type='checkbox' value='"&rs("ID")&"'></td>" & vbCrLf
      Response.Write "</tr>" & vbCrLf
	  rs.movenext
    wend
    Response.Write "<tr>" & vbCrLf
    Response.Write "<td colspan='11' nowrap align=""right"" class=""forumRow""><input type=""submit"" name=""batch"" value=""中文生效"" onClick=""return test();""> <input type=""submit"" name=""batch"" value=""中文失效"" onClick=""return test();""> <input type=""submit"" name=""batch"" value=""英文生效"" onClick=""return test();""> <input type=""submit"" name=""batch"" value=""英文失效"" onClick=""return test();""> <input onClick=""CheckAll(this.form)"" name=""buttonAllSelect"" type=""button"" id=""submitAllSearch"" value=""全选""> <input onClick=""CheckOthers(this.form)"" name=""buttonOtherSelect"" type=""button"" id=""submitOtherSelect"" value=""反选""> <input name='batch' type='submit' value='删除所选' onClick=""return test();""></td>" & vbCrLf
    Response.Write "</tr>" & vbCrLf
  else
    response.write "<tr><td nowrap align='center' colspan='11' class=""forumRow"">暂无企业新闻</td></tr>"
  end if
  Response.Write "<tr>" & vbCrLf
...
Function TextPath(ID)
  ...
  rs.open sql,conn,1,1
  TextPath=" -> <a href=""ImageList.asp?SortID="&rs("ID")&"&SortPath="&rs("SortPath")&""">"&rs("SortNameCh")&"</a>"
